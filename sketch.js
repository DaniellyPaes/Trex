...
function setup() {
  ...
  invisibleGround = createSprite(width / 2, height - 17, width, 10); // Cria o sprite do solo invisível
...
function draw() {
  ...
  trex.collide(invisibleGround); // Faz o dinossauro colidir com o solo invisível
...
function spawnObstacles() {
  if (frameCount % 60 === 0) {
    let obstacle = createSprite(400, height - 45, 10, 40); // Cria um obstáculo
    ...
    obstacle.lifetime = 200; // Define o tempo de vida do obstáculo
...
function spawnClouds() {
  if (frameCount % 60 === 0) {
    cloud = createSprite(600, 100, 40, 10); // Cria uma nuvem
    cloud.y = Math.round(random(1, 100)); // Define a posição y da nuvem aleatoriamente
    ...
    cloud.lifetime = 200; // Define o tempo de vida da nuvem
